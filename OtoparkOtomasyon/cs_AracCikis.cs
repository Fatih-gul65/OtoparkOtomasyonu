...
namespace OtoparkOtomasyon
{
    internal class cs_AracCikis
    {
        ...
        public cs_AracCikis(cs_Baglanti baglanti, TextBox txtAracPlakasi, TextBox txtDogrulamaKodu, Label lblTutar, Label lblKalinanSure, RadioButton rdbtnNakit, RadioButton rdbtnKrediKart)
        {
            _baglanti = baglanti;
        ...
        public void Hesapla()
        {
            try
            {
                ...
                {
                    cs_MesajGoster.Hata("Bu plakaya ait çıkış işlemi zaten yapılmış!");
                    return;
                }
                ...
                if (girisKaydi == null)
                {
                    cs_MesajGoster.Hata("Bu plakaya ait giriş kaydı bulunamadı!");
                    return;
                }

                if (!_rdbtnNakit.Checked && !_rdbtnKrediKart.Checked)
                {
                    cs_MesajGoster.Uyari("Lütfen bir ödeme türü seçiniz!");
                    return;
                }
                // Ücretsiz Giriş Kontrolü
                if (entities.UcretsizGiris.Any(u => u.Plaka == plaka))
                {
                    cs_MesajGoster.Bilgi("Bu Araç Ücretsiz Giriş Listesinde, Ücretsiz Çıkış.");
                    _lblTutar.Text = "0 TL";
                ...
                {
                    cs_MesajGoster.Hata("Ücret tarifesi bulunamadı!");
                    return;
                }
                ...
                {
                    cs_MesajGoster.Bilgi("Süresi Dolmuş Abonelik. Normal Ücret.");
                }
                else
                {
                    // Araç hâlâ abone mi?
                    ...
                    {
                        cs_MesajGoster.Bilgi("Abone Araç, Ücretsiz Çıkış.");
                        _lblTutar.Text = "0 TL";
                        _lblKalinanSure.Text = $"{Math.Ceiling(kalinanSure.TotalHours)} saat";
                        _hesaplandiMi = true;
                        return;
        ...
        public void Kaydet()
        {
            try
            {
                if (!_hesaplandiMi)
                {
                    cs_MesajGoster.Uyari("Lütfen önce ücreti hesaplayınız!");
                    return;
                }
                ...
                cs_MesajGoster.Bilgi("Çıkış başarıyla kaydedildi!");
